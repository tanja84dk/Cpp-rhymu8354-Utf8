...
mod tests {
    #[test]
    fn ascii_to_unicode() {
        assert_eq!(Ok("Hello"), std::str::from_utf8(b"Hello"));
    }

    #[test]
    fn encode_ascii() {
        assert_eq!(b"Hello", "Hello".as_bytes());
    }

    #[test]
    fn symbols() {
        assert_eq!(b"\x41\xe2\x89\xa2\xce\x91\x2e", "A≢Α.".as_bytes());
        assert_eq!(b"\xe2\x82\xac", "€".as_bytes());
    }
    ...
    fn stump_of_tree_encoding() {
        assert_eq!(b"\xf0\xa3\x8e\xb4", "𣎴".as_bytes());
    }

    #[test]
    fn code_point_beyond_end_of_last_valid_range() {
        assert_eq!(None, std::char::from_u32(0x20_0000_u32));
        assert_eq!(None, std::char::from_u32(0x11_0000_u32));
    }
    ...
    fn high_and_low_surrogate_halves_are_invalid() {
        ...
        assert_eq!(b"\xed\x9f\xbf", &buffer);
        assert_eq!(None, std::char::from_u32(0xd800));
        assert_eq!(None, std::char::from_u32(0xd801));
        assert_eq!(None, std::char::from_u32(0xd803));
        assert_eq!(None, std::char::from_u32(0xdfef));
        assert_eq!(None, std::char::from_u32(0xdffe));
        assert_eq!(None, std::char::from_u32(0xdfff));
        assert_eq!(
            3,
            std::char::from_u32(0xe000).unwrap().encode_utf8(&mut buffer).len()
        );
        assert_eq!(b"\xee\x80\x80", &buffer);
    }
    ...
    fn decode_valid_sequences() {
        ...
        let test_vectors = [
            TestVector {
                encoding: "𣎴",
                expected_decoding: &[0x233B4],
            },
            TestVector {
                encoding: "日本語",
                expected_decoding: &[0x65E5, 0x672C, 0x8A9E],
            },
            TestVector {
                encoding: "A≢Α.",
                expected_decoding: &[0x0041, 0x2262, 0x0391, 0x002E],
            },
            TestVector {
                encoding: "€",
                expected_decoding: &[0x20AC],
            },
            TestVector {
                encoding: "Hello",
                expected_decoding: &[0x48, 0x65, 0x6C, 0x6C, 0x6F],
            },
        ];
    ...
    fn reject_overlong_sequences() {
        ...
        let test_vectors = [
            ...
            TestVector {
                encoding: b"\xe0\x80\xaf",
                expected_decoding: "���",
            },
            TestVector {
                encoding: b"\xf0\x80\x80\xaf",
                expected_decoding: "����",
            },
            ...
            TestVector {
                encoding: b"\xe0\x9f\xbf",
                expected_decoding: "���",
            },
            TestVector {
                encoding: b"\xf0\x8f\xbf\xbf",
                expected_decoding: "����",
            },
        ];
        for test_vector in &test_vectors {
    ...
    fn stump_of_tree_decoded_in_two_parts() {
        assert_eq!((None, 2), bstr::decode_utf8(b"\xf0\xa3"));
        assert_eq!((Some('𣎴'), 4), bstr::decode_utf8(b"\xf0\xa3\x8e\xb4"));
    }
